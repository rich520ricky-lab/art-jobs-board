...
§
AI Salesbot (sale.aiceox.com) — Taiwan market. Sellers upload products → auto-generate product copy, photos (downloadable), QR sales codes linked to Taiwan mobile payment, ad placement. GitHub: github.com/rich520ricky-lab/ai-sales-bot (private). Deployed on Oracle Cloud. PHP 8, Apache, MariaDB. Managed via aaPanel — do NOT touch Apache/httpd configs via SSH. DB credentials: see .env (REDACTED from public repo). Google OAuth from settings table. Status as of 2026-05-05: deployed & working, OAuth configured.
§
